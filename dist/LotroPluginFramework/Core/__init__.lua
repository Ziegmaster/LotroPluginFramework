...
LPFClasses.LotroPluginFramework = _G.class()
...
function LPFClasses.LotroPluginFramework:Constructor()
	...
	self.PluginGlobals = {}
	self.Dict = LPFClasses.LPFDict:Constructor(self)
	--[[
    	General LPF Settings
	]]
	self.Settings = LPFClasses.LPFSettings:Constructor(self)
	--[[
    	General localization object
	]]
	self.Texts = LPFClasses.LPFTexts:Constructor(self)
	self.Shell = LPFClasses.LPFShell:Constructor(self)
	self.Events = LPFClasses.LPFEvents:Constructor(self)
	self.PluginData = LPFClasses.LPFPluginData:Constructor(self)
	self.Modules = LPFClasses.LPFModuleDict:Constructor(self)
	return self
end
...
function LPFClasses.LotroPluginFramework:InitPlugin(init_args)
	...
	if init_args.PluginData and type(init_args.PluginData) == "table" and init_args.PluginData.Enabled == true then
		if init_args.PluginData.DataScope == nil then
			self.Settings.Plugin = self.PluginDataLoad(Turbine.DataScope.Account, "")
			...
			if type(init_args.PluginData.DataScope) == "number" then
				self.Settings.Plugin = self.PluginDataLoad(init_args.PluginData.DataScope, "")
				if self.Settings.Plugin == nil then
					self.Settings.FirstLaunch = true
					self.Settings.Plugin = self.Settings.PluginDefaults
					self.Shell.DebugMessage(
	...
	self.Events.AddListener(plugin, "Unload", function()
		if init_args.PluginData and type(init_args.PluginData) == "table" and init_args.PluginData.Enabled == true then
			...
			self.PluginDataSave(init_args.PluginData.DataScope, "", self.Settings.Plugin)
